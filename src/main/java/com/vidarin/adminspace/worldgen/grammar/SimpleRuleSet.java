...
public class SimpleRuleSet<P, S> implements RuleSet<P, S> {
    private final Int2ObjectMap<List<Rule<P, S>>> rules = new Int2ObjectOpenHashMap<>();
    private int size = 0;
    ...
    public boolean add(Rule<P, S> rule) {
        if (rule.predecessor().symbol().isTerminal()) throw new IllegalArgumentException("Rules cannot have terminal predecessors");
        if (rules.containsKey(rule.predecessor().symbol().identifier())) {
            List<Rule<P, S>> list = rules.get(rule.predecessor().symbol().identifier());
    ...
    public boolean contains(Object o) {
        if (o instanceof Symbol symbol) {
            ...
            if (rules.containsKey(rule.predecessor().symbol().identifier())) {
                return rules.get(rule.predecessor().symbol().identifier()).contains(rule);
            }
        }
        return false;
    }
