...
public class DimensionSkySector extends WorldProvider {
    ...
    public void onWorldUpdateEntities() {
        ...
        for (EntityPlayerMP player : players) {
            if (ticksInDimension.get(player.getUniqueID()) % 4000 == 0) {
    ...
    private void playDimensionMusic(EntityPlayerMP player) {
        Random rand = new Random((long) (player.posX + player.posY + player.posZ));
    ...
    private void updatePlayerSettings(EntityPlayer player) {
        if (player.getUniqueID().equals(Minecraft.getMinecraft().player.getUniqueID())) {
            ...
            if (Minecraft.getMinecraft().gameSettings.renderDistanceChunks > 5)
                Minecraft.getMinecraft().gameSettings.renderDistanceChunks = 5;
